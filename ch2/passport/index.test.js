...
it("serializeUser 유저 아이디를 done 콜백으로 호출해야 한다", () => {
  const done = jest.fn();
  jest
    .spyOn(passport, "serializeUser")
    .mockImplementation((cb) => cb({ id: 1 }, done));
  jest.spyOn(passport, "deserializeUser").mockImplementation();
...
it("deserializeUser 유저 객체를 done 콜백으로 호출해야 한다", () => {
  const done = jest.fn();
...
it("deserializeUser 유저 조회 시 에러가 나면 done 콜백으로 에러를 호출해야 한다", () => {
  const done = jest.fn();
