...
beforeAll(async () => {
  // DB 연결, 테이블 초기화
  await sequelize.sync({ force: true }); // { force: true }: 데이터 베이스 테이블 초기화
});
...
describe("POST /auth/login", () => {
  ...
  test("가입하지 않은 회원이 로그인하려는 경우 실패 알리기", (done) => {
    agent
      ...
      .expect(
        "Location",
        "/?error=%EA%B0%80%EC%9E%85%EB%90%98%EC%A7%80%20%EC%95%8A%EC%9D%80%20%ED%9A%8C%EC%9B%90%EC%9E%85%EB%8B%88%EB%8B%A4.",
      )
      .expect(302, done);
  });
...
describe("POST /auth/logout", () => {
  ...
  it("로그인을 안 했는데 로그아웃하면 /로 돌려보낸다", (done) => {
    expect.assertions(1); // expect 횟수 검사
    agent
      .get("/auth/logout")
      .expect(403)
      .then((response) => {
